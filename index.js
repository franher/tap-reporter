...
function Formatter(type, options) {
  ...
  if (!reporters[type]) {
    ...
    throw new Error('Unknown format type: %s\n\n%s', type, avail);
  }
  ...
  runner.on('end', function () {
    if (!runner.parser.ok) exitCode = 1;
  });
}

process.on('exit', function (code) {
  if (!code && exitCode) process.exit(exitCode);
});
...
function avail() {
  var types = Formatter.types
    .reduce(function (str, t) {
      var ll = str.split('\n').pop().length + t.length;
      if (ll < 40) return str + ' ' + t;
      else return str + '\n' + t;
    }, '')
    .trim();
...
if (require.main === module) {
  ...
  process.stdin.pipe(
    new Formatter(type, {
      reporterOptions: { output: outputPath, prependTestFileName },
    })
  );
}
